...
public class Test {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		int[] n_prime= {16};//the number of signers in ring signature.
		int[] n= {100,200,300,400,500,600,700,800,900,1000};//the number of users in the system.
		...
		phasepos=0;
		kpos=0;
		for(npos=0;npos<n_prime.length;npos++) {
			for(wpos=0;wpos<w.length;wpos++) {
				System.out.println("n="+n_prime[npos]+", w="+w[wpos]);
				executeSM(n_prime[npos], w[wpos],repeat,data,phasepos,kpos,npos,wpos);
			}
		}
		...
		for(int j=0;j<data[0].length;j++) {
			...
			System.out.println("");
			for(int l=0;l<n_prime.length;l++) {
		...
		for(npos=0;npos<n.length;npos++) {
			...
			executeGW(n[npos],repeat,data,phasepos,kpos,npos,wpos);
		}
		...
		for(kpos=0;kpos<k.length;kpos++) {
			for(npos=0;npos<n.length;npos++) {
				for(wpos=0;wpos<w.length;wpos++) {
					...
					executeDC(k[kpos], n[npos], w[wpos],repeat,data,phasepos,kpos,npos,wpos);
				}
			}
		}
